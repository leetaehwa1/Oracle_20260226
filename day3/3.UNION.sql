...
-- 3. 정렬(ORDER BY) 는 맨 마지막 줄에 작성. 첫 번째 쿼리 컬럼을 기준으로 한다.

SELECT STU_NAME FROM STUDENT
UNION
SELECT COUNT(*) FROM STUDENT; --컬럼의 데이터 타입이 달라서 오류발생

SELECT STU_NAME ZZZ FROM STUDENT
UNION
SELECT STU_DEPT FROM STUDENT; -- 컬럼의 이름은 첫 번째 쿼리를 따른다
...
-- 학생 별 시험 평균 점수 출력 + 전체 학생의 평균 점수 출력
SELECT 
    STU_NAME, 
