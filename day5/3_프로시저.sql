...
CREATE OR REPLACE PROCEDURE EMPSAL_PROC(I_EMPNO IN EMP.EMPNO%TYPE, I_SAL IN EMP.SAL%TYPE)
IS
    O_COUNT NUMBER;
BEGIN
    UPDATE EMP
...
/
SELECT * FROM EMP;
EXEC EMPSAL_PROC(7654,-500);
ROLLBACK;


-- 프로시저 호출
-- ENROL_PROC('학번','과목번호','수정할 점수')
-- 1. 없는 학번이나 없는 과목 번호를 입력하면 '정보를 다시 확인해주세요' 출력
-- 2. 점수가 0미만, 100 초과일 경우 '점수의 범위는 1~100입니다' 출력
...
IS
    O_STU_NO ENROL.STU_NO%TYPE;
    O_SUB_NO ENROL.SUB_NO%TYPE;
    O_ENR_GRADE ENROL.ENR_GRADE%TYPE;
    O_COUNT NUMBER;
BEGIN
...
SELECT * FROM ENROL;
EXEC ENROL_PROC(20131001,104,80);