...
--)
SELECT M.MEMBER_ID,MEMBER_NAME,SUM(ORDER_QTY)SUM_QTY
FROM MEMBER M
...
--)
SELECT MAX(ORDER_PRICE),MIN(ORDER_PRICE)
FROM PRODUCT_ORDERS;

SELECT MAX(SUM_PRICE) - MIN(SUM_PRICE)
FROM(
    SELECT  
        MEMBER_ID,
        SUM(ORDER_PRICE) AS SUM_PRICE
        FROM PRODUCT_ORDERS
        GROUP BY MEMBER_ID
    );
...
SELECT M.MEMBER_ID , MEMBER_NAME,ROUND(AVG(ORDER_PRICE),1) AVG_PRICE
FROM PRODUCT_ORDERS O
INNER JOIN MEMBER M ON O.MEMBER_ID = M.MEMBER_ID
GROUP BY M.MEMBER_ID,MEMBER_NAME
HAVING AVG(O.ORDER_PRICE) > (
    SELECT AVG(ORDER_PRICE)
    FROM PRODUCT_ORDERS)
;
...
SELECT *
FROM(
    SELECT E1.EMPNO,E1.ENAME,COUNT(E2.MGR) 부하
    FROM EMP E1 
...
GROUP BY
    ...
    SELECT
    CASE 
        WHEN SUBSTR(JUMIN, 7,1) IN('1','3') AND SUBSTR(JUMIN, 7,1) IN('2','4') THEN '남자''여자'
    END AS 성,
    COUNT(*) AS 인원
FROM STU
GROUP BY
