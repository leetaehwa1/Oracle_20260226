...
SELECT C.NAME,ROUND(AVG(SALEPRICE),1)
FROM CUSTOMER C 
INNER JOIN ORDERS O ON C.CUSTID = O.CUSTID
GROUP BY O.CUSTID ,C.NAME; --O.CUSTID -> PK값도 그룹함수로 포함시켜야함.
--7. 이상미디어의 책을 구매한 고객 중에서 같은 성(姓)을 가진 사람이 몇 명이나 되는지 성별 인원수를 구하시오.
SELECT SUBSTR(C.NAME,1,1) || '씨' 성,  COUNT(*) AS 인원수
FROM BOOK B
...
INNER JOIN ORDERS O ON B.BOOKID = O.BOOKID
INNER JOIN CUSTOMER C ON O.CUSTID = C.CUSTID
WHERE PUBLISHER = '이상미디어'  AND TO_CHAR(ORDERDATE, 'YY-MM-DD') = '20-07-07'
GROUP BY ORDERID,ORDERDATE,NAME,B.BOOKID;
--HAVING TO_CHAR(ORDERDATE, 'YY-MM-DD') = '20-07-07';
--9. 이름, 전화번호가 포함된 고객목록을 보이시오. 단, 전화번호가 없는 고객은 ‘연락처없음’으로 표시하시오.
SELECT NAME, DECODE(PHONE, NULL, '연락처없음', PHONE) AS PHONE
FROM CUSTOMER;
...
--11. ‘대한민국’에 거주하는 고객에게 판매한 도서의 총 판매액을 출력하시오. 46000원
...
SELECT E1.ENAME, ABS(S2.GRADE - S1.GRADE)
FROM EMP E1
...
SELECT ENAME, DNAME, SAL, ROUND(AVG(SAL))
FROM EMP E
INNER JOIN ( 
...
INNER JOIN DEPT D ON E.DEPTNO = T.DEPTNO
WHERE SAL > AVG_SAL
;
